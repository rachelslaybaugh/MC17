...
Let us write \cref{i} in the mathematically equivalent form
\bala
...
\nal
where $\bl s\bg_\ep$ is $O(1)$.

Next, we define $\psi(\ux,\uom,s)\equiv\f{\ep\sue}{e^{-\int_0^s\Sigma_t(s')ds'}}\Psi(\ux,\uom,\ep s)$.
This satisfies
\bsub\label[pluraleq]{v}
\bal
&\f{\partial \psi}{\partial s}(s) + \ep\uom \cdot \unab \psi(s)= 0, \qquad s>0,\label{va}\\
&\psi(0) = \label{vb}\\
...
Integrating \cref{va} over $0<s'<s$ and using \cref{vb}, we obtain
\bala%\label{2.9}
...
\nala
for $n=0,1,2,...$ , we integrate \cref{viii} over the unit sphere and obtain
\bala
...
\bsub\label[pluraleq]{2.15}
\bal
&\left(I- \ep^2 \setb - \f{4\ep^4}{5}\setb^2 - \f{44\ep^6}{35}\setb^3 + O(\ep^8)\right)\vi =\label{2.15a}\\
...
We now multiply \cref{2.15} by $e^{-\int_0^s\Sigma_t(s')ds'}/\sue$ and operate on them by $\int_0^\infty (\cdot) ds$. Using \cref{ii,iv,viii}, \cref{2.15b} yields an expression for the scalar flux:
\bala
...
\Cref{2.15a} can be rewritten as
\bal\label{2.18}
...
Finally, rearranging the terms in \cref{2.18} we get
\bal\label{2.19}
\left(\sum_{n=0}^\infty \ep^{2n}\left[W_{n+1}\unab_0^{n+1}+\kappa U_n\unab_0^n\right]\right)\Phi(\ux) = \sue q(\ux),
\nal
where $W_{n}=V_n-U_n$.
If we discard the terms of $O(\ep^{2n+2})$ in this equation, we obtain a partial differential equation for $\Phi(\ux)$ of order $2n$.
We will use this approach to explicitly derive the nonclassical SP$_1$, SP$_2$, and SP$_3$ equations.
Higher-order equations can be derived from \cref{2.19} by continuing to follow the same procedure.

\subsection{Nonclassical diffusion equation (nonclassical SP$_1$)}
We discard the terms of $O(\ep^2)$ in \cref{2.19} and rewrite the equation as 
\bala%\label{2.20}
...
Discarding the terms of $O(\ep^4)$ in \cref{2.19} and rearranging the terms, we get
\bala%\label{2.24}
...
Discarding the terms of $O(\ep^6)$ in \cref{2.19}, we have
\bala
...
Finally, we multiply the two equations above by $\ep$ and revert to the original unscaled parameters to obtain the nonclassical SP$_3$ equations
\bsub\label[pluraleq]{2.32}
\bal
&-\f{1}{6}\f{\sd}{\su}\unab^2\bigg[\big[1+\beta_1(1-c)\big]\Phi(\ux) + 2\nu(\ux)\bigg] + \label{2.32a}\\
&\hspace{15em}\f{1-c}{\su}\Phi(\ux) = Q(\ux),\nonumber\\
&-\f{1}{6}\f{\sd}{\su}\unab^2\left[\f{\lambda_1}{2}\Phi(\ux)+\lambda_2\nu(\ux)\right]+\f{1-\beta_2(1-c)}{\su}\nu(\ux) = 0,\label{2.32b}
\nal
\nsub
where $\lambda_1$ and $\beta_1$ are the same as in the SP$_2$ equations,
\bala
\lambda_2 =& \f{1}{10\sd\st-9\su\sq }\times\\
&\quad \left[\f{9}{5}\sc - \f{27}{21}\f{\su\ss}{\sd} + 3\f{\st\sq}{\sd} - \f{10}{3} \f{\st^2}{\su}\right],
\nala
and $\beta_2 = \f{1}{10\sde\ste-9\sue\sqe}\left[\f{10}{3}\f{\ste^2}{\sue} - \f{9}{5}\sce\right]-1$.
...
Introducing this result into the nonclassical diffusion approximation given by \cref{2.22}, one can easily see that it reduces to the classical diffusion equation
\bala
- \f{1}{3\Sigma_t}\unab^2\Phi(\ux) + \Sigma_a\Phi(\ux) = Q(\ux).
\nala
We also obtain $\lambda_1 = \f{4}{5}$; $\lambda_2 = \f{11}{7}$; and $\beta_1 = \beta_2 = 0$.
In this case, the nonclassical SP$_2$ equation (\ref{2.26a}) reduces to
\bal\label{3.1}
- \f{1}{3\Sigma_t}\unab^2&\left[\Phi(\ux) + \f{4}{5}\f{\Sigma_a\Phi(\ux)- Q(\ux)}{\Sigma_t}\right] + \Sigma_a\Phi(\ux) = Q(\ux),
\nal
which is the SP$_2$ approximation to \cref{1.6b} \cite{mcl11}.
The nonclassical SP$_3$ equations (\ref{2.32a}) and (\ref{2.32b}) reduce to 
\bsub\label[pluraleq]{3.2}
\bal
-\f{1}{3\Sigma_t}\unab^2\bigg[\Phi(\ux) + 2\nu(\ux)\bigg] + \Sigma_a\Phi(\ux) = Q(\ux),\label{3.2a}\\
-\f{1}{3\Sigma_t}\unab^2\left[\f{2}{5}\Phi(\ux)+\f{11}{7}\nu(\ux)\right]+\Sigma_t\nu(\ux) = 0,\label{3.2b}
\nal
\nsub
which are the classical SP$_3$ equations \cite{mcl11}.

Moreover, $U_1=U_2=U_3...=0$ and $V_0 = 1$; $V_1 = -\f{1}{\sigma_t^2}$; $V_2 = -\f{4}{5\sigma_t^4}$; $V_3 = -\f{44}{35\sigma_t^6}$; with $\sigma_t = \Sigma_t/\ep$.
Thus, \cref{2.19} becomes
\bala\label{3.3}
...
\nala
This is the asymptotic approximation to the classical transport equation that can be used to obtain the classical SP$_N$ equations \cite{mcl11}.  
...
\nala
with $\wst = 2\f{\su}{\sd}$ and $\wsa = \f{1-c}{\su}$. Similarly, the nonclassical SP$_2$ equation becomes
\bala
...
Finally, the nonclassical SP$_3$ equations become
\Cref{i} becomes
\bala
